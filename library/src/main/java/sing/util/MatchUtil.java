...
public class MatchUtil {
    ...
    public static boolean isCarId(String str) {
        Pattern pattern = Pattern.compile(car_id);
        Matcher m = pattern.matcher(str);
        return m.matches();
    }
    // 判断是否为邮政编码
     ...
     */
    public static boolean isIDCard(String IDStr) throws RuntimeException {
        String[] ValCodeArr = { "1", "0", "x", "9", "8", "7", "6", "5", "4", "3", "2" };
        String[] Wi = { "7", "9", "10", "5", "8", "4", "2", "1", "6", "3", "7", "9", "10", "5", "8", "4", "2" };
        String Ai = "";
        // ================ 号码的长度 15位或18位 ================
        if (IDStr.length() != 15 && IDStr.length() != 18) {
            LogUtil.e("身份证号码长度应该为15位或18位");
            return false;
        }
        ...
        if (isNumeric(Ai) == false) {
            LogUtil.e("身份证15位号码都应为数字; 18位号码除最后一位外，都应为数字。");
            return false;
        }
        ...
        if (IDStr.length() == 18) {
            if (modValue == 2) {
                if ((Ai + "x").equals(IDStr) == false && (Ai + "X").equals(IDStr) == false) {
                    LogUtil.e("身份证不合法");
                    return false;
                }
            }else{
                Ai = Ai + ValCodeArr[modValue];
